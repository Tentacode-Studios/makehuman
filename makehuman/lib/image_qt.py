...
def load(path):
    ...
    # `fromstring` was removed in binary mode in numpy; use frombuffer instead
...
def toQImage(data):
    ...
    if d == 1:
        ...
        fmt = QtGui.QImage.Format_RGB32
        pixels = (
            0xFF000000 + data[..., 0] * 0x10000 + data[..., 1] * 0x100 + data[..., 2]
        )
    elif d == 4:
        fmt = QtGui.QImage.Format_ARGB32
        pixels = (
    ...
    # `tostring()` is deprecated, use `tobytes()` for clarity
...
def resized(img, width, height, filter=0):
    ...
    qi = qi.scaled(QtCore.QSize(int(width), int(height)), transformMode=transform)
    return load(qi)
